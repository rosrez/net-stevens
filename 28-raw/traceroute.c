...
int     max_ttl = 30;                   /* maximum TTL to use, can be overridden with -m option */
int     nprobes = 3;
u_short dport = 32768 + 666;            /* destination port #; gets incremented after each attempt */
...
int main(int argc, char *argv[])
{
    ...
    if ((ai = host_serv(host, NULL, 0, 0)) == NULL)
        err_quit("can't get address info for %s", host);
